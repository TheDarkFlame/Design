//source code from http://www.itriacasa.it/generalized-hough-transform/GHT.html
#include <vector>
...
class GenHoughTrnf {
private:
	Mat accum; // accumulator matrix
	Mat showimage; // accumulator matrix

	std::vector<Rpoint> pts;// contour points:
	Vec2i refPoint;// reference point (inside contour)
	
	std::vector<std::vector<Vec2i>> Rtable;// R-table of template object:
	
	int intervals;// number of intervals for angles of R-table:
	int thr1;// lower threshold of canny edge detector
	int thr2;// upper threshold of canny edge detector

	int wtemplate;// width of template contour
	int wmin;// minimum width of scaled contour
	int wmax;// maximum width of scaled contour

	float phimin;// minimum rotation allowed for template
	float phimax;// maximum rotation allowed for template

	int rangeXY;// dimension in pixels of squares in image
	int rangeS;// interval to increase scale
...
public:
	...
	void createTemplate();// save file with canny edge of the original image
	void createRtable();
	void accumulate(cv::Mat& input_img);// fill accumulator matrix
...
void GenHoughTrnf::createTemplate() {
...
void GenHoughTrnf::createRtable() {
	// code can be improved reading a pre-saved Rtable
	readPoints();
	readRtable();
}

void GenHoughTrnf::accumulate(cv::Mat& input_img) {
	showimage = input_img;
